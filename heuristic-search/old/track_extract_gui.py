...
def extract_track(filename, manhattan_heuristic):
	...
	### connects states to those that represent velocities in a different direction	
	### target_states are 1 step from origin_states		    
	def connect_track_side(origin_states, target_states):
	    [s0, s1, s2] = origin_states
	    [t0, t1, t2] = target_states
	    ### first s0: possible actions, stay or accelerate
	    a1 = action(3, [1], [s0]) ### stay
	    a2 = action(6, [1], [t1]) ### accelerate
	    s0.append_action(a1)
	    s0.append_action(a2)
	    ### second s1: possible actions, descelerate, stay, or accelerate
	    a1 = action(6, [1], [t0]) ### descelerate
	    a2 = action(3, [1], [t1]) ### stay
	    a3 = action(6, [1], [t2]) ### accelerate
	    s1.append_action(a1)
	    s1.append_action(a2)
	    s1.append_action(a2)
	    ### third s2: possible actions, descelerate, stay
	    a1 = action(6, [1], [t1]) ### descelerate
	    s2.append_action(a1)
	    origin_states = [s0, s1, s2]
	    target_states = [t0, t1, t2]
	    return (origin_states, target_states)
	...
	### connects states to those that represent velocities in a different direction	    
	### target_states are 2 steps from origin_states
	def connect_track_double_side(origin_states, target_states):
	    [s0, s1, s2] = origin_states
	    [t0, t1, t2] = target_states
	    ### first s0: possible actions, stay or accelerate
	    ### there are no possible actions
	...
	for index in index_to_states:
		...
		if index+width in index_to_states:
	        ...
	        if index + width not in index_to_states:
		    ### it colided with wall, put speed zero
	            a = 1 #r = connect_track_break(origin_states)
		...
		if index + 1 in index_to_states:
		    ...
		    r = connect_track_side(origin_states, right_states)
		    ...
		    right_states = right_states[0:1] + right_states[5:7]
		    r = connect_track_double_side(origin_states, right_states)
		...
		if index - 1 in index_to_states:
		    ...
		    r = connect_track_side(origin_states, left_states)
		...
		if index - width in index_to_states:    
		    upper_states = index_to_states[index-width]
		    upper_states = upper_states[0:1] + upper_states[1:3]
		    r = connect_track_side(origin_states, upper_states)
		    ...
		    upper_states = index_to_states[index-width]
		    upper_states = upper_states[0:1] + upper_states[1:3]
		    r = connect_track_double_side(origin_states, upper_states)
		...
		if index+width in index_to_states:
		    lower_states = index_to_states[index+width]
		    lower_states = lower_states[0:1] + lower_states[3:5]
		    r = connect_track_side(origin_states, lower_states)
		    ...
		    lower_states = index_to_states[index+2*width]
		    lower_states = lower_states[0:1] + lower_states[3:5]
		    r = connect_track_double_side(origin_states, lower_states)
		...
		if index + 1 in index_to_states:
		    ...
		    right_states = index_to_states[index+2]
		    right_states = right_states[0:1] + right_states[5:7]
		    a=1#r = connect_track_double(origin_states, right_states)
